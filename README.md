# Lancy - AI 作品集门户

个人作品集入口门户网站，展示多个 AI 工作流项目。

## 项目介绍

这是 `lancy.site` 的主入口站，展示我的各个 AI 工作流项目：

- 🎬 [AI 漫剧工作流](https://comic.lancy.site) - AI 漫剧自动生成
- 🛒 [电商生视频工作流](https://ecommerce.lancy.site) - 商品短视频 AI 生成
- 📢 [公众号一键优化工作流](https://blog.lancy.site) - 公众号内容创作工具
- 🔍 [降AI率图文](https://noai.lancy.site) - 自动优化图文内容，降AI率
- 📈 [AI 量化交易](https://quant.lancy.site) - 基于 AI 的量化交易系统

## 技术栈

- 纯静态 HTML + CSS，部署在 GitHub Pages。

## 部署

- 主域名 `lancy.site` 通过 GitHub Pages 托管。
- 各子域名对应不同项目独立部署。

## License

MIT License
